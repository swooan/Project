...
(function($, window, document, undefined) {
	...
	var methods = {
		...
		addListeners : function() {
			...
			this.$stars.on('click', this.hoverRating.bind(this));
		},
		...
		getIndex : function(e) {
			...
			index = (minRating && minRating <= this.settings.totalStars && index < minRating) ? minRating - 1
					: index;
			return index;
		},
		...
		getOffsetByPixel : function(e, $target, width) {
			...
			return (leftX <= (width / 2) && !this.settings.useFullStars) ? 'left'
					: 'right';
		},
		...
		paintStars : function(endIndex, stateClass) {
			...
			$
					.each(
							this.$stars,
							function(index, star) {
								$polygonLeft = $(star).find(
										'[data-side="left"]');
								$polygonRight = $(star).find(
										'[data-side="right"]');
								leftClass = rightClass = (index <= endIndex) ? stateClass
										: 'empty';

								// has another half rating, add half star
								leftClass = (index - endIndex === 0.5) ? stateClass
										: leftClass;

								$polygonLeft.attr('class', 'svg-' + leftClass
										+ '-' + this._uid);
								$polygonRight.attr('class', 'svg-' + rightClass
										+ '-' + this._uid);

								// get color for level
								var ratedColorsIndex = endIndex >= 0 ? Math
										.ceil(endIndex) : 0;

								var ratedColor;
								if (s.ratedColors && s.ratedColors.length
								...
								// only override colors in rated stars and when
								// rated number is valid
								if (stateClass === 'rated' && endIndex > -1) {
									// limit to painting only to rated stars,
									// and specific case for half star
									if (index <= Math.ceil(endIndex)
											|| (index < 1 && endIndex < 0)) {
		...
		renderMarkup : function() {
			...
			var star = '<div class="jq-star" style="width:'
					+ s.starSize
					+ 'px;  height:'
					+ s.starSize
					...
					this.getLinearGradient(this._uid + '_SVGID_1_',
							s.emptyColor, s.emptyColor, s.starShape)
					+ this.getLinearGradient(this._uid + '_SVGID_2_',
							s.hoverColor, s.hoverColor, s.starShape)
					+ this.getLinearGradient(this._uid + '_SVGID_3_',
							s.starGradient.start, s.starGradient.end,
							s.starShape) + this.getVectorPath(this._uid, {
		...
		getVectorPath : function(id, attrs) {
			return (attrs.starShape === 'rounded') ? this.getRoundedVectorPath(
					id, attrs) : this.getSpikeVectorPath(id, attrs);
		},

		getSpikeVectorPath : function(id, attrs) {
			return '<polygon data-side="center" class="svg-empty-'
					+ id
		...
		getRoundedVectorPath : function(id, attrs) {
			...
			return '<path data-side="center" class="svg-empty-'
					+ id
					+ '" d="'
					+ fullPoints
					+ '" style="stroke: '
		...
		getSvgDimensions : function(starShape) {
			return (starShape === 'rounded') ? 'width="550px" height="500.2px" viewBox="0 146.8 550 500.2" style="enable-background:new 0 0 550 500.2;'
	...
	$.fn[pluginName] = function(options) {
		...
		if (!$.isPlainObject(options)) {
			if (publicMethods.hasOwnProperty(options)) {
				return publicMethods[options].apply(this, Array.prototype.slice
						.call(arguments, 1));
			} else {
				$.error('Method ' + options + ' does not exist on '
						+ pluginName + '.js');
			}
		}
