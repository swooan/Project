      ...
      <div id="content_list">
         ...
         <%            
            for(int i=0;i<shoplist.size();i++){
               ...
               if(shopvo.getShop_idx()%3 == 1){
         %>
            <div class="shop_first">
               <div class="list_img">
                  <a class="img_a" href="detail?shopidx=<%=shopvo.getShop_idx() %>" style="background-image: url(<c:url value="${path}/res/image/default.png"/>)">
                     <div class="img_top">
                        <span>예약 <%=shopvo.getShop_reserve()%> 리뷰 <%=shopvo.getShop_review()%> 조회수<%=shopvo.getShop_view()%></span>
               ...
               <div class="shop_desc">
                  <div class="shop_star"><span class="star_span" id="shop_tmp<%=shopvo.getShop_idx()%>"></span><span class="shop_score"><%=shopvo.getShop_score() %>점</span></div>
                  <div class="shop_budget"><%=shopvo.getBudget()%></div>
                  <div class="shop_comment"><%=shopvo.getShop_comment() %></div>
               </div>
               <div class="review_btn">
                  <a class="review_a" href="detail?shopidx=<%=shopvo.getShop_idx()%>">리뷰 쓰기</a>
               </div>
            </div>
            <%
               }else{ 
